...
void GS_GL20SC_PFO_BFS_ROBUSTNESS_TC_002(void) {
  ...
  for (int i = 0; i < combo_count; i++) {
    glBlendFuncSeparate(combos[i].srcRGB, combos[i].dstRGB, combos[i].srcAlpha,
                        combos[i].dstAlpha);
...
void GS_GL20SC_PFO_BFS_ROBUSTNESS_TC_003(void) {
  ...
  for (mode = 0; mode < 65536; mode++) {
    GLenum expected = is_valid_factor(mode) ? GL_NO_ERROR : GL_INVALID_ENUM;
    ...
    if (err != expected) {
      TEST_LOG_FAIL(test_case_3, test_procedure,
                    "srcRGB=0x%X Beklenen=0x%X Gelen=0x%X", mode, expected,
                    err);
      failCount++;
    } else {
      passCount++;
  ...
  TEST_LOG_INFO("srcRGB sweep - PASS: %d, FAIL: %d", passCount, failCount);

  /* Varsayilana geri don */
  glBlendFuncSeparate(GL_ONE, GL_ZERO, GL_ONE, GL_ZERO);

  if (failCount != 0) {
    TEST_LOG_FAIL(test_case_3, test_procedure, "Basarisiz cagri sayisi: %d",
                  failCount);
    return;
  }
...
void GS_GL20SC_PFO_BFS_ROBUSTNESS_TC_004(void) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    /* State korunmasi */
    if (!check_blend_state(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE,
                           GL_ZERO)) {
      TEST_LOG_FAIL(test_case_4, test_procedure, "State bozuldu: Enum=0x%X",
                    extreme[i]);
      return;
    }
  }
...
void GS_GL20SC_PFO_BFS_ROBUSTNESS_TC_005(void) {
  ...
  struct {
    GLenum srcRGB, dstRGB, srcAlpha, dstAlpha;
      ...
      {GL_DST_COLOR, GL_CONSTANT_ALPHA, GL_SRC_ALPHA, GL_ONE_MINUS_DST_ALPHA},
  };
  ...
  for (int i = 0; i < combo_count; i++) {
    glBlendFuncSeparate(valid_combos[i].srcRGB, valid_combos[i].dstRGB,
                        valid_combos[i].srcAlpha, valid_combos[i].dstAlpha);
    GLenum err = glGetError();
    if (err != GL_NO_ERROR) {
    ...
    for (int j = 0; j < inv_count; j++) {
      /* Gecersiz srcRGB */
      glBlendFuncSeparate(invalidEnums[j], GL_ONE, GL_ONE, GL_ONE);
      if (glGetError() != GL_INVALID_ENUM) {
        TEST_LOG_FAIL(test_case_5, test_procedure, "srcRGB hatasi: inv=0x%X",
                      invalidEnums[j]);
        return;
      }

      /* Gecersiz dstRGB */
      glBlendFuncSeparate(GL_ONE, invalidEnums[j], GL_ONE, GL_ONE);
      if (glGetError() != GL_INVALID_ENUM) {
        TEST_LOG_FAIL(test_case_5, test_procedure, "dstRGB hatasi: inv=0x%X",
                      invalidEnums[j]);
        return;
      }

      /* Gecersiz srcAlpha */
      glBlendFuncSeparate(GL_ONE, GL_ONE, invalidEnums[j], GL_ONE);
      if (glGetError() != GL_INVALID_ENUM) {
        TEST_LOG_FAIL(test_case_5, test_procedure, "srcAlpha hatasi: inv=0x%X",
                      invalidEnums[j]);
        return;
      }

      /* Gecersiz dstAlpha */
      glBlendFuncSeparate(GL_ONE, GL_ONE, GL_ONE, invalidEnums[j]);
      if (glGetError() != GL_INVALID_ENUM) {
        TEST_LOG_FAIL(test_case_5, test_procedure, "dstAlpha hatasi: inv=0x%X",
                      invalidEnums[j]);
        return;
      }

      /* State korunmasi */
      if (!check_blend_state(valid_combos[i].srcRGB, valid_combos[i].dstRGB,
                             valid_combos[i].srcAlpha,
                             valid_combos[i].dstAlpha)) {
        TEST_LOG_FAIL(test_case_5, test_procedure,
                      "State bozuldu: kombo=%d, inv=0x%X", i, invalidEnums[j]);
        return;
      }
    }
  }
  ...
  for (int i = 0; i < 50000; i++) {
    ...
    if (i % 5 == 0) {
      ...
      for (int p = 0; p < 4; p++) {
        ...
        params[p] = valid_factors[(seed >> 16) % valid_count];
      }
    } else {
      /* Rastgele (buyuk olasilikla gecersiz) */
      for (int p = 0; p < 4; p++) {
        seed = seed * 1103515245 + 12345;
        params[p] = (GLenum)((seed >> 16) % 0x10000);
      }
    }
