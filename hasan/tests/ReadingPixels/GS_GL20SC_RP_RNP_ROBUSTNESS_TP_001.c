...
/*
GL20SC - ReadingPixels - ReadnPixels - ROBUSTNESS
*/

static const char *test_procedure = "GS_GL20SC_RP_RNP_ROBUSTNESS_TP_001";
static const char *test_case_1 = "GS_GL20SC_RP_RNP_ROBUSTNESS_TC_001";
static const char *test_case_2 = "GS_GL20SC_RP_RNP_ROBUSTNESS_TC_002";
static const char *test_case_3 = "GS_GL20SC_RP_RNP_ROBUSTNESS_TC_003";
static const char *test_case_4 = "GS_GL20SC_RP_RNP_ROBUSTNESS_TC_004";
...
void GS_GL20SC_RP_RNP_ROBUSTNESS_TP_001_close(void);

// --- TEST 1: YETERSİZ KUTU BOYUTU (Buffer Overflow) ---
void GS_GL20SC_RP_RNP_ROBUSTNESS_TC_001(void) {
...
void GS_GL20SC_RP_RNP_ROBUSTNESS_TC_002(void) {
  while (glGetError() != GL_NO_ERROR)
    ;
  ...
  if (err == GL_INVALID_VALUE) {
    ...
    TEST_LOG_FAIL(test_case_2, test_procedure,
                  "Expected GL_INVALID_VALUE (0x501), but got 0x%X", err);
  }
}

// --- TEST 3: GEÇERSİZ PARAMETRE (Fuzzing) ---
void GS_GL20SC_RP_RNP_ROBUSTNESS_TC_003(void) {
  while (glGetError() != GL_NO_ERROR)
    ;
  ...
  if (err == GL_INVALID_ENUM) {
    ...
    TEST_LOG_FAIL(test_case_3, test_procedure,
                  "Expected GL_INVALID_ENUM (0x500), but got 0x%X", err);
  }
}

// --- TEST 4: EKRAN DIŞI KOORDİNAT (Out of Bounds) ---
void GS_GL20SC_RP_RNP_ROBUSTNESS_TC_004(void) {
  while (glGetError() != GL_NO_ERROR)
    ;
  ...
  if (err == GL_NO_ERROR) {
    ...
    TEST_LOG_FAIL(test_case_4, test_procedure,
                  "Unexpected error for out of bounds coordinates: 0x%X", err);
  }
}

/* Initialization */
void GS_GL20SC_RP_RNP_ROBUSTNESS_TP_001_init(void) {
  ...
  GS_GL20SC_RP_RNP_ROBUSTNESS_TC_001();
  // CHECK_ERROR(test_procedure);
  GS_GL20SC_RP_RNP_ROBUSTNESS_TC_002();
  // CHECK_ERROR(test_procedure);
  GS_GL20SC_RP_RNP_ROBUSTNESS_TC_003();
  // CHECK_ERROR(test_procedure);
  GS_GL20SC_RP_RNP_ROBUSTNESS_TC_004();
  // CHECK_ERROR(test_procedure);

  GS_GL20SC_RP_RNP_ROBUSTNESS_TP_001_close();
}

void GS_GL20SC_RP_RNP_ROBUSTNESS_TP_001_draw(void) {}
