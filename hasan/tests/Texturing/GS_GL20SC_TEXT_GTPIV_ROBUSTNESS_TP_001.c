...
/*
 * GL20SC - Texturing - GetTexParameteriv - ROBUSTNESS
 *
 * glGetTexParameteriv robustness testleri.
...
static const char *test_procedure = "GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TP_001";
static const char *test_case_1 = "GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TC_001";
static const char *test_case_2 = "GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TC_002";
static const char *test_case_3 = "GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TC_003";
static const char *test_case_4 = "GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TC_004";

void GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TP_001_close(void);
...
void GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TC_001(void) {
  ...
  for (int i = 0; i < tgt_count; i++) {
    glGetTexParameteriv(bad_targets[i], GL_TEXTURE_MAG_FILTER, &param);
  ...
  for (int i = 0; i < prof_count; i++) {
    glGetTexParameteriv(profile_targets[i], GL_TEXTURE_MAG_FILTER, &param);
...
// Doku parametresi sorgulama isleminde, glGetTexParameteriv yalnizca spesifikasyonda tanimli
...
void GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TC_002(void) {
  while (glGetError() != GL_NO_ERROR);
  GLint param = 0;
  int fail_count = 0;
  ...
  for (int i = 0; i < pnm_count; i++) {
    glGetTexParameteriv(GL_TEXTURE_2D, bad_pnames[i], &param);
    GLenum err = glGetError();
...
void GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TC_003(void) {
  while (glGetError() != GL_NO_ERROR);
  int fail_count = 0;

  glGetTexParameteriv(GL_TEXTURE_3D, GL_TEXTURE_MAG_FILTER, NULL);
  GLenum err = glGetError();
  ...
  glGetTexParameteriv(GL_TEXTURE_2D, GL_COMPILE_STATUS, NULL);
  err = glGetError();
...
// glGetTexParameteriv normal sartlarda params pointer'ina tek bir GLint (4 bayt) yazar.
...
void GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TC_004(void) {
  while (glGetError() != GL_NO_ERROR);
  int fail_count = 0;

  GLint buffer[3];
  buffer[0] = 0xDEADBEEF; // Koruma (Guard)
  buffer[1] = 0;          // Output (Target)
  buffer[2] = 0xDEADBEEF; // Koruma (Guard)

  glGetTexParameteriv(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, &buffer[1]);
  GLenum err = glGetError();
  ...
  if (buffer[0] != (GLint)0xDEADBEEF || buffer[2] != (GLint)0xDEADBEEF) {
...
void GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TP_001_init(void) {
  glGenTextures(1, &g_tex);
  glBindTexture(GL_TEXTURE_2D, g_tex);
  while (glGetError() != GL_NO_ERROR);

  GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TC_001();
  GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TC_002();
  GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TC_003();
  GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TC_004();

  GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TP_001_close();
}

void GS_GL20SC_TEXT_GTPIV_ROBUSTNESS_TP_001_draw(void) {}
