...
/*
 * GL20SC - Texturing - TexParameteri - ROBUSTNESS
 *
 * glTexParameteri robustness testleri.
...
static const char *test_procedure = "GS_GL20SC_TEXT_TPI_ROBUSTNESS_TP_001";
static const char *test_case_1 = "GS_GL20SC_TEXT_TPI_ROBUSTNESS_TC_001";
static const char *test_case_2 = "GS_GL20SC_TEXT_TPI_ROBUSTNESS_TC_002";
static const char *test_case_3 = "GS_GL20SC_TEXT_TPI_ROBUSTNESS_TC_003";

void GS_GL20SC_TEXT_TPI_ROBUSTNESS_TP_001_close(void);

static GLuint g_tex = 0;

// TC_001: Gecersiz target
// SC 2.0 spesifikasyonuna gore glTexParameteri yalnizca belli basli target'lari (ozellikle TEXTURE_2D)
...
void GS_GL20SC_TEXT_TPI_ROBUSTNESS_TC_001(void) {
  ...
  for (int i = 0; i < tgt_count; i++) {
    glTexParameteri(bad_targets[i], GL_TEXTURE_MAG_FILTER, GL_NEAREST);
  ...
  for (int i = 0; i < prof_count; i++) {
    glTexParameteri(profile_targets[i], GL_TEXTURE_MAG_FILTER, GL_NEAREST);
...
// Doku parametresi isleminde, glTexParameteri yalnizca spesifikasyonda tanimli
...
void GS_GL20SC_TEXT_TPI_ROBUSTNESS_TC_002(void) {
  while (glGetError() != GL_NO_ERROR);
  int fail_count = 0;
  ...
  for (int i = 0; i < pnm_count; i++) {
    glTexParameteri(GL_TEXTURE_2D, bad_pnames[i], GL_NEAREST);
    GLenum err = glGetError();
...
void GS_GL20SC_TEXT_TPI_ROBUSTNESS_TC_003(void) {
  while (glGetError() != GL_NO_ERROR);
  int fail_count = 0;
  GLenum err;
  ...
  for (int i = 0; i < sizeof(bad_wrap) / sizeof(bad_wrap[0]); i++) {
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, bad_wrap[i]);
    err = glGetError();
    if (err != GL_INVALID_ENUM) {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, bad_wrap[i]);
    err = glGetError();
    if (err != GL_INVALID_ENUM) {
  ...
  for (int i = 0; i < sizeof(bad_min) / sizeof(bad_min[0]); i++) {
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, bad_min[i]);
    err = glGetError();
    if (err != GL_INVALID_ENUM) {
  ...
  // MIN_FILTER de gecerli olan MIPMAP filtreleri MAG_FILTER'de GECERSIZDIR.
  ...
  for (int i = 0; i < sizeof(bad_mag) / sizeof(bad_mag[0]); i++) {
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, bad_mag[i]);
    err = glGetError();
    if (err != GL_INVALID_ENUM) {
...
void GS_GL20SC_TEXT_TPI_ROBUSTNESS_TP_001_init(void) {
  glGenTextures(1, &g_tex);
  glBindTexture(GL_TEXTURE_2D, g_tex);
  while (glGetError() != GL_NO_ERROR);

  GS_GL20SC_TEXT_TPI_ROBUSTNESS_TC_001();
  GS_GL20SC_TEXT_TPI_ROBUSTNESS_TC_002();
  GS_GL20SC_TEXT_TPI_ROBUSTNESS_TC_003();

  GS_GL20SC_TEXT_TPI_ROBUSTNESS_TP_001_close();
}

void GS_GL20SC_TEXT_TPI_ROBUSTNESS_TP_001_draw(void) {}
