...
void Texturing_GenTextures_TC_001(void) {
  ...
  if (!after_gen && after_bind && !after_delete && err == GL_NO_ERROR)
    ...
    TEST_LOG_FAIL(test_case1, test_procedure, "Durum zinciri bozuldu: gen=%d, bind=%d, del=%d, err=0x%X", after_gen, after_bind, after_delete, err);
}
...
void Texturing_GenTextures_TC_007(void) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 2, 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, red_pixels);
...
void Texturing_GenTextures_TC_008(void) {
  ...
  if (err == GL_INVALID_VALUE)
    ...
    TEST_LOG_FAIL(test_case8, test_procedure, "Beklenen GL_INVALID_VALUE (0x501), alinan: 0x%X", err);
}
...
void Texturing_GenTextures_TC_010(void) {
  ...
  if (err == GL_INVALID_VALUE)
    ...
    TEST_LOG_FAIL(test_case10, test_procedure, "Beklenen GL_INVALID_VALUE (0x501), alinan: 0x%X", err);
}
...
void Texturing_GenTextures_TC_011(void) {
  ...
  if (err == GL_NO_ERROR || err == GL_INVALID_VALUE || err == GL_INVALID_OPERATION)
    TEST_LOG_SUCCESS(test_case11, test_procedure);
  else
    TEST_LOG_FAIL(test_case11, test_procedure, "Sistem cökmedi veya beklenmeyen hata: 0x%X", err);
}
