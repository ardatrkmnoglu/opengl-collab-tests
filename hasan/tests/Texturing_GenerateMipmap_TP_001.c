...
void Texturing_GenerateMipmap_TC_003(void) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    GLubyte *data = create_solid_color_data(npot_sizes[i].w, npot_sizes[i].h, 0,
                                            0, 255, 255);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, npot_sizes[i].w, npot_sizes[i].h, 0,
...
void Texturing_GenerateMipmap_TC_004(void) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, test_size, test_size, 0, GL_RGBA,
...
void Texturing_GenerateMipmap_TC_007(void) {
  ...
  if (mip_err == GL_NO_ERROR || mip_err == GL_INVALID_OPERATION ||
    ...
    TEST_LOG_FAIL(test_case7, test_procedure, "0x0 texture mipmap hatasi: 0x%X",
                  mip_err);
}
...
void Texturing_GenerateMipmap_TC_008(void) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (formats[i].format == GL_RGB)
      components = 3;
    else if (formats[i].format == GL_LUMINANCE || formats[i].format == GL_ALPHA)
      components = 1;
...
void Texturing_GenerateMipmap_TC_009(void) {
  ...
  if (crash_resilient)
    ...
    TEST_LOG_FAIL(test_case9, test_procedure, "VRAM stresi yonetilemedi");
...
void Texturing_GenerateMipmap_TC_010(void) {
  ...
  if (err == GL_INVALID_OPERATION || err == GL_NO_ERROR)
    ...
    TEST_LOG_FAIL(test_case10, test_procedure,
                  "Bind edilmemis texture mipmap hatasi: 0x%X", err);
}
...
void Texturing_GenerateMipmap_TC_012(void) {
...
#endif
    TEST_LOG_FAIL(test_case12, test_procedure, "FBO tamamlanmadi (0x%X)",
                  fbo_status);
    return;
  }
  ...
  if (err == GL_NO_ERROR && r > 200 && g < 50 && b < 50)
    ...
    TEST_LOG_FAIL(test_case12, test_procedure,
                  "Piksel kirmizi degil R=%d G=%d B=%d veya hata=0x%X", r, g, b,
                  err);
...
void Texturing_GenerateMipmap_TC_013(void) {
  ...
  for (int i = 0; i < 6; i++) {
    glTexImage2D(faces[i], 0, GL_RGBA, 64, 64, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                 data);
  }
  free(data);
...
void Texturing_GenerateMipmap_TC_014(void) {
  ...
  glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, GL_RGBA, 32, 32, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, data);
  glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, GL_RGBA, 32, 32, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, data);
  glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, GL_RGBA, 32, 32, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, data);
  free(data);
...
void Texturing_GenerateMipmap_TC_015(void) {
  ...
  if (err1 == GL_NO_ERROR && err2 == GL_NO_ERROR)
    ...
    TEST_LOG_FAIL(test_case15, test_procedure,
                  "Mipmap yenileme hatali: err1=0x%X, err2=0x%X", err1, err2);
}
