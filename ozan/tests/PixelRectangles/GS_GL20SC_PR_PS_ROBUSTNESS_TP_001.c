	...
	for (i = 0; i < 4; i++) {
		...
		if (err != GL_NO_ERROR) {
			TEST_LOG_FAIL(test_case_1, test_procedure,
				      "PACK_ALIGNMENT=%d Error=0x%X",
				      validValues[i], err);
			return;
		}

		if (!checkIntState(test_case_1, test_procedure,
				   GL_UNPACK_ALIGNMENT, validValues[i]))
			return;
	...
	for (pname = 0; pname < 10000; pname++) {
		...
		if (err == GL_INVALID_ENUM) {
			...
			TEST_LOG_FAIL(test_case_3, test_procedure,
				      "pname=0x%X Beklenmeyen Error=0x%X",
				      pname, err);
			return;
		}
	}
...
void GS_GL20SC_PR_PS_ROBUSTNESS_TC_004(void) {
	...
	if (err != GL_NO_ERROR) {
		TEST_LOG_FAIL(test_case_4, test_procedure,
			      "PACK_ALIGNMENT=8 Error=0x%X", err);
		return;
	}
	...
	if (value != 8) {
		TEST_LOG_FAIL(test_case_4, test_procedure,
			      "PACK_ALIGNMENT beklenen=8 gercek=%d", value);
		return;
	}
	...
	if (err != GL_INVALID_VALUE) {
		TEST_LOG_FAIL(test_case_4, test_procedure,
			      "Beklenen=GL_INVALID_VALUE Gelen=0x%X", err);
		return;
	}
	...
	if (value != 8) {
		TEST_LOG_FAIL(test_case_4, test_procedure,
			      "PACK_ALIGNMENT durumu bozuldu. Gercek=%d",
			      value);
		return;
	}
	...
	if (err != GL_NO_ERROR) {
		TEST_LOG_FAIL(test_case_4, test_procedure,
			      "UNPACK_ALIGNMENT=2 Error=0x%X", err);
		return;
	}
	...
	if (value != 2) {
		TEST_LOG_FAIL(test_case_4, test_procedure,
			      "UNPACK_ALIGNMENT beklenen=2 gercek=%d", value);
		return;
	}
	...
	if (value != 2) {
		TEST_LOG_FAIL(test_case_4, test_procedure,
			      "UNPACK_ALIGNMENT durumu bozuldu. Gercek=%d",
			      value);
		return;
	}
...
void GS_GL20SC_PR_PS_ROBUSTNESS_TC_005(void) {
	...
	if (err != GL_NO_ERROR) {
		TEST_LOG_FAIL(test_case_5, test_procedure,
			      "Beklenen=GL_NO_ERROR Gelen=0x%X", err);
		return;
	}
	...
	if (err != GL_INVALID_ENUM) {
		TEST_LOG_FAIL(test_case_5, test_procedure,
			      "Beklenen=GL_INVALID_ENUM Gelen=0x%X", err);
		return;
	}
...
/* Initialization */
void GS_GL20SC_PR_PS_ROBUSTNESS_TP_001_init(void) {

	CHECK_ERROR(test_procedure);
	GS_GL20SC_PR_PS_ROBUSTNESS_TC_001();
	CHECK_ERROR(test_procedure);
	GS_GL20SC_PR_PS_ROBUSTNESS_TC_002();
	CHECK_ERROR(test_procedure);
	GS_GL20SC_PR_PS_ROBUSTNESS_TC_003();
	CHECK_ERROR(test_procedure);
	GS_GL20SC_PR_PS_ROBUSTNESS_TC_004();
	CHECK_ERROR(test_procedure);
	GS_GL20SC_PR_PS_ROBUSTNESS_TC_005();
	CHECK_ERROR(test_procedure);
	GS_GL20SC_PR_PS_ROBUSTNESS_TC_006();
	CHECK_ERROR(test_procedure);


}

void GS_GL20SC_PR_PS_ROBUSTNESS_TP_001_draw(void) {
}
/* Cleanup */
void GS_GL20SC_PR_PS_ROBUSTNESS_TP_001_close(void) {
