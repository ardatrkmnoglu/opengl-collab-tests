...
void GS_GL20SC_VC_V_ROBUSTNESS_TC_001(void) {
	...
	if (err != GL_NO_ERROR) {
		TEST_LOG_FAIL(test_case_1, test_procedure,
			      "Gecerli viewport reddedildi.");
		return;
	}

	glGetIntegerv(GL_VIEWPORT, viewport);

	if (viewport[0] != 10 || viewport[1] != 20 || viewport[2] != 640 ||
	    viewport[3] != 480) {
		TEST_LOG_FAIL(test_case_1, test_procedure,
			      "Viewport state hatali.");
		return;
	}
...
void GS_GL20SC_VC_V_ROBUSTNESS_TC_003(void) {
	...
	for (i = 0; i < count; i++) {
		...
		if (viewport[0] != coordinates[i][0] ||
		    viewport[1] != coordinates[i][1]) {

			TEST_LOG_FAIL(
			    test_case_3, test_procedure,
			    "Viewport koordinatlari dogru saklanmadi. "
			    "Beklenen: (%d,%d) Gercek: (%d,%d)",
			    coordinates[i][0], coordinates[i][1], viewport[0],
			    viewport[1]);
			return;
		}
	}
...
void GS_GL20SC_VC_V_ROBUSTNESS_TC_004(void) {
	...
	if (err != GL_NO_ERROR) {
		TEST_LOG_FAIL(test_case_4, test_procedure,
			      "GL_MAX_VIEWPORT_DIMS okunamadi (0x%X)", err);
		return;
	}
...
void GS_GL20SC_VC_V_ROBUSTNESS_TC_005(void) {
	...
	if (err != GL_INVALID_VALUE) {
		TEST_LOG_FAIL(test_case_5, test_procedure,
			      "Beklenen=0x%X Gelen=0x%X", GL_INVALID_VALUE,
			      err);
		return;
	}
	...
	if (glGetError() != GL_NO_ERROR) {
		TEST_LOG_FAIL(test_case_5, test_procedure,
			      "Gecerli viewport sonrasi hata olustu");
		return;
	}
...
/* Initialization */
void GS_GL20SC_VC_V_ROBUSTNESS_TP_001_init(void) {

	CHECK_ERROR(test_procedure);
	GS_GL20SC_VC_V_ROBUSTNESS_TC_001();
	CHECK_ERROR(test_procedure);
	GS_GL20SC_VC_V_ROBUSTNESS_TC_002();
	CHECK_ERROR(test_procedure);
	GS_GL20SC_VC_V_ROBUSTNESS_TC_003();
	CHECK_ERROR(test_procedure);
	GS_GL20SC_VC_V_ROBUSTNESS_TC_004();
	CHECK_ERROR(test_procedure);
	GS_GL20SC_VC_V_ROBUSTNESS_TC_005();
	CHECK_ERROR(test_procedure);
	GS_GL20SC_VC_V_ROBUSTNESS_TC_006();
	CHECK_ERROR(test_procedure);

}

void GS_GL20SC_VC_V_ROBUSTNESS_TP_001_draw(void) {
}
/* Cleanup */
void GS_GL20SC_VC_V_ROBUSTNESS_TP_001_close(void) {
